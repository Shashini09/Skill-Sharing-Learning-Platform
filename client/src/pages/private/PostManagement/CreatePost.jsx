...
const CreatePost = () => {
  ...
  const isFormValid = user?.id && post.topic && topicWordCount <= 20 && post.description && descriptionWordCount <= 50;
  ...
  const startListening = () => {
    ...
    recognition.start();
    window.recognitionInstance = recognition; // Make globally accessible to stop later
  };
    ...
    <div className="p-6 max-w-2xl mx-auto bg-white rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <textarea name="topic" value={post.topic} onChange={handleInputChange}
        ...
        <div className="relative">
          ...
          <textarea name="description" value={post.description} onChange={handleInputChange}
            ...
            className="w-full p-3 border rounded-lg pr-28" />
          ...
          <div className="absolute right-3 top-12 flex items-center space-x-2">
            <button type="button" onClick={startListening} disabled={isListening}
              className={`text-sm px-3 py-1 rounded-lg ${isListening ? 'bg-gray-300 text-gray-600' : 'bg-blue-100 hover:bg-blue-200 text-blue-700'}`}>
              🎤 Speak
            </button>
        ...
        <div className="flex gap-3 items-center">
          <input name="location" value={post.location} onChange={handleInputChange}
            placeholder="Location will auto-fill..." className="w-full p-3 border rounded-lg" />
          <button type="button" onClick={detectLocation} className="text-sm bg-gray-200 px-3 py-2 rounded-lg hover:bg-gray-300">📍</button>
        </div>
        ...
        <div onDrop={handleFileDrop} onDragOver={handleDragOver}
          ...
          <input type="file" multiple accept=".jpg,.jpeg,.png,.mp4"
            onChange={handleFileSelect} className="w-full mt-3" />
        </div>
            ...
            {mediaFiles.map((p, i) => (
              <div key={i} className="relative border p-2 rounded-lg group">
                <button type="button" onClick={() => handleRemoveFile(i)}
                  className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 text-xs">❌</button>
                {i > 0 && <button type="button" onClick={() => moveFile(i, -1)}
                  className="absolute left-2 top-2 bg-blue-500 text-white p-1 text-xs">⬆️</button>}
                {i < mediaFiles.length - 1 && <button type="button" onClick={() => moveFile(i, 1)}
                  className="absolute left-2 bottom-2 bg-blue-500 text-white p-1 text-xs">⬇️</button>}
        ...
        <input type="text" name="taggedFriends" placeholder="Tagged friends (comma separated)"
          onChange={(e) => setPost({ ...post, taggedFriends: e.target.value.split(',').map(f => f.trim()) })}
          className="w-full p-3 border rounded-lg" />

        <button type="submit" disabled={!isFormValid}
          className={`w-full py-3 rounded-lg text-white font-semibold ${isFormValid ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-400 cursor-not-allowed'}`}>
          Create Post
        </button>
      </form>
