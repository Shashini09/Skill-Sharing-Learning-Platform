...
const PostFeed = () => {
  ...
  const fetchComments = async (postId) => {
    try {
      const res = await axios.get(`http://localhost:8080/api/comments/${postId}`, { withCredentials: true });
  ...
  const handleUpdateComment = async (commentId, postId) => {
    try {
      await axios.put(`http://localhost:8080/api/comments/${commentId}`, { text: editCommentText }, { withCredentials: true });
      setComments((prev) => ({
        ...prev,
        ...
        <div className="bg-white rounded-xl shadow-sm mb-8 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${filter === 'myPosts' 
                ? 'bg-blue-600 text-white' 
                : 'bg-gray-200 hover:bg-gray-300 text-gray-800'}`}
            >
              My Posts
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${filter === 'recent' 
                ? 'bg-blue-600 text-white' 
                : 'bg-gray-200 hover:bg-gray-300 text-gray-800'}`}
            >
              Recent
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${filter === 'popular' 
                ? 'bg-blue-600 text-white' 
                : 'bg-gray-200 hover:bg-gray-300 text-gray-800'}`}
            >
              Popular
                ...
                {editingPostId === post.id ? (
                  <div className="p-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <label htmlFor="taggedFriends" className="block text-sm font-medium text-gray-700 mb-1">Tagged Friends</label>
                        <input
                          id="taggedFriends"
                          name="taggedFriends"
                          value={editValues.taggedFriends}
                          onChange={handleEditChange}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
...
{visibleComments[post.id] && (
  <div className="mt-6 border-t border-gray-300 pt-6">
    <form onSubmit={(e) => handleCommentSubmit(e, post.id)} className="mb-6">
      <textarea
      ...
      {(comments[post.id] || []).map((comment) => (
        ...
        >
          <div className="text-sm text-gray-600 mb-2 flex justify-between items-center">
            <span>
              <strong className="text-blue-700">{comment.user}</strong> ·{' '}
