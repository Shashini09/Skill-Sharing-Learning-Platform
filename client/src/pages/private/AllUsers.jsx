...
export default function AllUsers() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (validUsers.length < fetchedUsers.length) {
          console.warn(
            "Some users were excluded due to missing or invalid IDs:",
            fetchedUsers
          );
        }
        ...
        const updatedUsers = validUsers.map((user) => ({
          ...user,
          isFollowed:
            currentUser?.following?.includes(user.id.toString()) || false,
        }));
        ...
        setError(
          "Failed to fetch users. Please check the server or try again later."
        );
        setLoading(false);
      }
    };
  ...
  const handleFollowToggle = async (userId, isFollowed) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data ||
        `Could not ${isFollowed ? "unfollow" : "follow"} user: Unknown error`;
  ...
  const filteredUsers = users
    .filter((dbUser) => {
      ...
      if (
        ...
        console.log(
          `Comparing dbUser.id=${
            dbUser.id
          } (${typeof dbUser.id}) with currentUser.id=${
            currentUser.id
          } (${typeof currentUser.id}): ${
            isNotCurrentUser ? "Excluding" : "Including"
          }`
        );
    ...
    .filter(
      (dbUser) =>
        ...
        (dbUser.email &&
          dbUser.email.toLowerCase().includes(searchTerm.toLowerCase()))
    )
    .filter((dbUser) => {
              ...
              >
                <div className="p-6">
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        handleFollowToggle(dbUser.id, dbUser.isFollowed)
                      }
